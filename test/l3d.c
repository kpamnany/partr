...
void l3d_partr(int nx, int padded_nx, int ny, int nz, float *u1, float *u2);
...
int main(int argc, char **argv)
{
    ...
    if (argc != 7) {
        fprintf(stderr, "Usage:\n"
            "  laplace3d <nx> <ny> <nz> <#iters> <naive|auto|sse|avx|partr> "
            "<verify?>\n");
        exit(-1);
    }
    ...
    if (strncasecmp(argv[5], "naive", 5) == 0)
        ...
        l3d = l3d_avx;
    else if (strncasecmp(argv[5], "partr", 5) == 0)
        l3d = l3d_partr;
    else {
        fprintf(stderr,
                "don't recognize %s. naive, auto, sse, avx, or partr?\n",
                argv[5]);
        exit(-1);
    }
    ...
    if (strncasecmp(argv[5], "partr", 5) == 0)
        partr_init();
